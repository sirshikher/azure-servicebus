...
import { MessageSession, SessionCallee } from "./messageSession";
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            while (!this._isCancelRequested) {
                try {
                    ...
                    log.sessionManager("[%s] Acquired the semaphore for max concurrent sessions: %d, %d.", connectionId, this._maxConcurrentSessionsSemaphore.currentLockCount(), this._maxConcurrentSessionsSemaphore.awaitedTaskCount());
                    yield this._maxPendingAcceptSessionsSemaphore.acquire();
                    log.sessionManager("[%s] Acquired the semaphore for max pending accept sessions: %d, %d.", connectionId, this._maxPendingAcceptSessionsSemaphore.currentLockCount(), this._maxPendingAcceptSessionsSemaphore.awaitedTaskCount());
                    const closeMessageSession = (messageSession) => __awaiter(this, void 0, void 0, function* () {
                        try {
                            ...
                            log.sessionManager("[%s] Releasing the semaphore for max concurrent sessions: %d, %d.", connectionId, this._maxConcurrentSessionsSemaphore.currentLockCount(), this._maxConcurrentSessionsSemaphore.awaitedTaskCount());
                            if (messageSession.isOpen()) {
                                yield messageSession.close();
                            }
                        }
                        catch (err) {
                            log.error("[%s] An error occurred while releasing the max concurrent session semaphore " +
                    ...
                    if (this._isCancelRequested) {
                        log.sessionManager("[%s] Since cancellation was requested, we will close the messageSession with id '%s'.", connectionId, messageSession.sessionId);
                    ...
                    const onSessionError = (error) => __awaiter(this, void 0, void 0, function* () {
                        log.sessionManager("An error ocurred in MessageSession with id '%s': %O. Hence closing it.", connectionId, sessionId, error);
                        yield closeMessageSession(messageSession);
                    ...
                    if (err.name === ConditionErrorNameMapper["amqp:operation-timeout"] ||
                        ...
                        if (!this._isCancelRequested) {
                            log.sessionManager("[%s] Sleeping for %d seconds, since there are no more active MessageSessions on " +
